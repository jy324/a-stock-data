...
- package 解禁映射跟进东财 `RPT_LIFT_STAGE` 新字段 `FREE_SHARES_TYPE` / `FREE_SHARES` / `ABLE_FREE_SHARES`，并兼容旧字段。
- package Eastmoney provider 对 HTTP 429/5xx 和连接级临时故障执行有限指数退避重试，403 保持立即失败并返回结构化 `unavailable`。
